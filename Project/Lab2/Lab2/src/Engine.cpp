...
void createShaderProgram()
{
	...
	shader = new Shader("shaders/bloomV.glsl", "shaders/bloomF.glsl");
	shader->load();
	shader->enable();
	shader->addAttribute(0, "aPos");
	...
	shaderBloomFinal->addAttribute(1, "aTexCoords");
...
void destroyShaderProgram()
{
	...
	shaderBlur->disable();
	glDeleteProgram(shaderBlur->getProgram());

	shaderBloomFinal->disable();
	glDeleteProgram(shaderBloomFinal->getProgram());
...
int main(int argc, char* argv[])
{
	...
	GLFWwindow* win = setup(gl_major, gl_minor,
		640, 480, "Interactive-Solar-System", is_fullscreen, is_vsync);
	run(win);
	exit(EXIT_SUCCESS);
}
